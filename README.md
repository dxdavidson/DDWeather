...
- `window.ASSET_VERSION = '20260412.2';`
...
When you change frontend code, update `window.ASSET_VERSION` in `index.html` to a new value, typically a date such as `20260412.2`, so users get the latest assets immediately.

## Webcam Image URLs
Webcam images are now read directly from the `url` values returned by `/api/webcam`.

No Netlify image proxy rewrite is used in the frontend.
